...
int main(const int argc, const char** argv)
{
	...
	if (exercise == 3)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < s; i++)
			{
				...
				//ans.data[i] = 3.f * v * v * v * (1 + v + v * v + v * v * v) + 3.f;
			}
			t.end();
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < s; i++)
			{
				//計算 yに書き込み
				const float v = x.data[i];
				y.data[i] = 3 * ((v * v * v * (v* (v * (v + 1) + 1) + 1)) +1);
			}
			t.end();
	...
	if (exercise == 4)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			{
				//計算 ansに書き込み
				ans.data[i] = ( 2 * 3.141592 + sqrt(5.0) + 0.5 * 0.5 ) * i;
		...
		for (int k = 0; k < loop; k++)
		{
			...
			t.start();
			const int s = x.rows * x.cols;
			const double const_4 =  2 * 3.141592 + sqrt(5.0) + 0.5 * 0.5; //定数値を計算
	...
	if (exercise == 5)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			//乗算にした場合
			t.start();
			const double const_5 = 1 / 3.141592;
			...
			for (int j = 0; j < x.rows; j++)
			{
				...
				{
					//計算
					y.data[j*x.cols + i] = x.data[j*x.cols + i] * const_5;
				}
			}
			t.end();
	...
	if (exercise == 6)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int j = 0; j < ret.rows; j++)
			{
				...
				{
					//計算 retに書き込み
					ret.data[j*ans.cols + i] = (a.data[j*ret.cols + i] * c.data[j*ret.cols + i]) / (b.data[j*ret.cols + i] * d.data[j*ret.cols + i]);
				}
			}
			t.end();
			//std::cout << "after : time: " << t.getLastTime() << " ms" << std::endl;
	...
	if (exercise == 7)
	{
		...
		for (int j = 0; j < loop; j++)
		{
			...
			for (int i = 0; i < s; i++)
			{
				ret.data[i] = x.data[i] * x.data[i] * x.data[i] * x.data[i] * x.data[i] * x.data[i] * x.data[i] *x.data[i] *x.data[i] * x.data[i];
			}
			t.end();
		}
	...
	if (exercise == 9)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < size; i++)
			{
			 	x_32s.data[i] = ret_32s.data[i] * 2.0f;
			}
			t.end();
		}
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < size; i++)
			{
				x_32s.data[i] = ret_32s.data[i] * 0.5;
			}
			t.end();
		}
	...
	if (exercise == 10)
	{
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < size; i++)
			{
				ret_32s.data[i] = x_32s.data[i] * 3.141f;
			}
			t.end();
		}
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < size; i++)
			{
				ret_32s.data[i] = (3216 * x_32s.data[i]) >> 10;
			}
			t.end();
		}
		...
		for (int k = 0; k < loop; k++)
		{
			...
			for (int i = 0; i < size; i++)
			{
				ret_16s.data[i] = (3216 * x_16s.data[i]) >> 10;
			}
			t.end();
		}
	...
	if (exercise == 18)
	{
		...
		std::cout << "|MP|" << t.getLastTime() << "|" << std::endl;
